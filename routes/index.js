...
router.post("/tokens", function(req, res){
    ...
    db.getAccountByUsername(username, function(errors, account){
        ...
        // Login succeeded.
        const access_token = jsonwebtoken.sign({accountId: account.id}, secretToken)
        
        const id_token = jsonwebtoken.sign({sub: account.id, preferred_username: account.username }, secretToken
      )
